    ...
    return (
        <section className="bg-black py-12">
            <div className="container mx-auto text-center">
                <h2 className="text-3xl font-bold text-red-600 mb-8 animate-pulse">Síguenos en redes sociales</h2>
                <div className="flex justify-center gap-5">
                    {socialLinks.map(link => (
                        <Link href={link.href} key={link.alt} target="_blank" rel="noopener noreferrer">
                            <Image src={link.src} alt={link.alt} width={80} height={80} className="object-contain transition-transform hover:scale-125" />
                        </Link>
                    ))}
                </div>
            </div>
